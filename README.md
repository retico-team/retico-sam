...
You should also install a checkpoint for whichever model version you would like to use. The default model for sam is 'vit_h' which can be found [here](https://dl.fbaipublicfiles.com/segment_anything/sam_vit_h_4b8939.pth). However for FastSAM the default model is 'YOLOv8x' which can be found [here](https://drive.google.com/file/d/1m1sjY4ihXBU1fZXdQ-Xdj-mDltW-2Rqv/view). This will start a download for that checkpoint. To use it you must include the path to the checkpoint when creating the sam module, as demonstrated in the example runner file. 
...
### SAMModule and FastSAMModule

**Model Options:** `'h' (vit_h)`, `'l' (vit_l)`, `'b' (vit_b)`

#### Arguments
The SAM module can take the following parameters: model, path_to_chkpnt, use_bbox, use_seg
* `model`(str): model type, no default.
NOTE: the user must match the model type with the corresponding checkpoint
* `path_to_chkpnt` (str): the path to the model checkpoint that was downloaded by the user, no default.
It is suggested to store the checkpoint file in the same directory as the runner file 
* `use_bbox` (bool): bounding box segmentation; Defaults to False
* `use_seg` (bool): segmentation mask; Defaults to False

### Hugging Face SAMModule

#### Arguments
The SAM module can take the following parameters: model, path_to_chkpnt, use_bbox, use_seg
* `model`(str): model type, defaults to `facebook/sam-vit-base`
* `show` (bool): visualization support, defaults to False
* `use_bbox` (bool): bounding box segmentation; Defaults to False
* `use_seg` (bool): segmentation mask; Defaults to False
...
* `num_obj_to_display` (int): number of extracted segmentation objects to display in the plot that will be saved to a folder named 'extracted_objects'. If the folder doesn't exist it will be created, otherwise it will just store the new image in the folder
```
import sys, os  
from retico import *  
